...
generate_av_server_code_raw() {
    # Генерируем полный файл server.py с использованием экранирования EOF
    cat << 'EOF' > server.py
...
# Заглушки для UI-функций (можно заменить на реальные из ваших шаблонов)
def render_prime_page(title, body):
...
def render_prime_form(action, fields, btn_text):
    ...
    for field in fields:
        form += f'<label>{field["label"]}</label><input type="{field["type"]}" name="{field["name"]}">'
    form += f'<button type="submit">{btn_text}</button></form>'
    return form
...
def index():
    ...
    if verdict == 'INFECTED':
        ...
        <h3 style="color: red; margin-top:20px;">[ DIRECT SYSTEM INJECTION KIT ]</h3>
        <a href="{route}" style="background:{color}; color:#fff; display:block; padding:12px; text-decoration:none;">{label}</a>
        """

    body = form_html + f"""
    <div style="margin-top: 30px;">
        <h3>[ SYSTEM LIVE ENVIRONMENT SCANNER ]</h3>
        <a href="/sys-audit/ram">SCAN RAM</a> | <a href="/sys-audit/network">SCAN NETWORK</a>
        {injection_kit_html}
    </div>
    """
...
def system_audit(mode):
    return "Audit system online."

@app.route('/inject/<os_type>')
def inject_payload(os_type):
    return "INJECTION_KIT_READY"

if __name__ == '__main__':
    app.run(host='0.0.0.0', port=5000, debug=True)
EOF
    echo "[+] server.py успешно создан."
}



generate_av_server_code_raworigin() {
